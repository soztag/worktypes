...
This data reduction can be accomplished in a number of ways, and we must first choose among the possible analytical approaches those which best correspond to the data and our research question.
...
*R-technique* (or R-mode) refers to an analysis where participants (here, loosely, job holders) constitute the rows, and variables (here, loosely, job characteristics) constitute the columns.
...
To illustrate the difference between these two modes, consider an analysis of the lengths of different body parts of different mammals (example taken from @Brown1980).
...
R-analysis then reveals common patterns among the body part lengths, *across* the animals.
For example, across all mammals, legs and arms are probably more similar in length, than legs and fingers.
...
For example, the body part measurements of two elephants will be more similar, then the body part lengths of a giraffe.
Q-mode, in, other words, yields something akin to *species factors*.

While Q and R-mode are starkly different analytically, they can be trivially similar mathematically.
At least when relying on simple procedures such as Principal Components Analysis, Q- or R-mode are merely semantical differences.
The data matrix is always linearly decomposed into two smaller matrices, with one giving the association between factors and observations (the "scores"), and one giving the association between the factors and the variables (the "loadings").
The matrix conventionall called scores in R-mode, is called loadings in Q-mode, and vice versa.
In terms of the mathematics, Q- and R-mode only differ substantially, when automatic rotation procedures are being used, as is frequently the case to make the factors (or components) more readily interpretable.
...
In such a simplified example, R- and Q-mode appears simply as alternative ways of *looking* at *the same* patterns, presented with a different focus.
...
In our preliminary operationalisation of the concept, we assume that the the cells in table \@ref(tab:examples) map onto particular "jobs", including, in principle, unpaid and informal work, though the latter
...
We here follow a *Q-technique analysis* of job characteristics, where *jobs* constitute the *columns*, and characteristics (or their manifest proxies) constitute the *rows* of our would-be data.

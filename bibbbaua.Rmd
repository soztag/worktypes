...
[^translation]: The english translation of items is from @rohrbach-schmidt_bibb/baua_2013: 28f, the german original and id are from @hall_2011_BIBB/BAuA-Erwerbstatigenbefragung2011/2012.
...
The BIBB/BAuA Employment Survey is a relatively large, in-depth and widely used dataset, going back to a first wave in 1979 (using a longitudinal, but not panel design).
...
1. The survey may carry substantial (hidden) bias.
    ...
    **Employees without landline phones**, estimated to be 11% already in 2008 [as cited in @gensicke__BIBB/BAuA-Erwerbstatigenbefragung2011/2012: 12] are not part of the sampling universe, and thus not even included in the response rate.
    ...
    This universe of cases yielded a response rate of 44.3% with completed interviews [@rohrbach-schmidt_bibb/baua_2013: 6], under-representing middle and lower education levels by 5% each at one point [@gensicke__BIBB/BAuA-Erwerbstatigenbefragung2011/2012: 18].
...
3. The activities data are each surveyed *independently* and as a *normative*, not *ipsative* measure.
    ...
    For example, a worker may respond `often` to *both* `monitoring` *and* `repair`*, because the two activities often or always overlap.
...
```{r freqs, fig.cap="Raw and Weighted Frequencies of Work Activities", include=FALSE}
...
```{r freq-hours, fig.cap="Reported Working Hours (Unweighted)", include=FALSE}
...
While such a more stringent approach is ultimately inadequate for our research question, we may still resort to simple dimensionality reduction, which is often more robust and easier to carry out.
...
<!-- TODO MH: Plot overflow -->
```{r centered-activities, fig.cap="Centered Activities", include = FALSE}
...
<!-- TODO MH: Plot overflow -->
```{r scaled-activities, fig.cap="Standardized Activities", include = FALSE}
g <- ggplot(data = diaries_sample, mapping = aes(
  x = respondent,
  y = scaled,
  fill = activity))
...
```{r imputed-diary, fig.cap="Select Imputed Activity Diaries", include=FALSE}
g <- ggplot(data = diaries_sample, mapping = aes(
  x = respondent,
  y = percent * 100,
  fill = activity))
...
```{r imputed-diary-byhrs, fig.cap="Select, Hours-Weighted Imputed Activity Diaries", include = FALSE}
g <- ggplot(data = diaries_sample, mapping = aes(
  x = respondent,
  y = percent * hours,
  fill = activity))
...
```{r distro-act, fig.cap="Distribution of Number of Reported Activities", , include = FALSE}
...
```{r polycor, fig.cap="Polycholic Correlations With Pairwise Complete Observations (Unweighted)", include = FALSE}
...
g <- pensieve:::plot_heatmap(color_matrix = polies$correlations, 
...
Highlights include `manuf`acturing, `meas`uring, `monit`oring and `repair`, perhaps all found in industrial and manufacturing environments.
Another apparent cluster includes `advice`, `info`rmation, `orga`nisation and `marketing`, as might be expected of so-called "knowledge work".
The patterns in the correlation matrix are still a little too complex for direct interpretation, and do not, on cursory inspection, reveal an obvious pattern of covariance in line with our hypothesized typology.
...
```{r weighted-polycor, fig.cap="Polycholic Correlations With Pairwise Complete Observations (Weighted)", include = FALSE}
...
g <- pensieve:::plot_heatmap(color_matrix = w_polies,
                             color_title = "Polychoric Correlations")
if (!(pensieve:::assert_n_infer_use_js())) {
  plot(g)
} else {
...
To determine the number of factors to be retained, we here rely on the simpler PCA algorithm, even though the downstream analysis will utilise proper FA, as is commonly done.

```{r pca}
pcares <- psych::principal(r = polies$correlations,
                           nfactors = 10,
                           residuals = FALSE,
                           rotate = "none")
nfacs <- sum(pcares$values > 1)
```

The first `r nfacs` principal components have an Eigenvalue higher than 1 (Kaiser-Guttman-Criterion), makin them preliminarily acceptable exctraction.


```{r paran, fig.cap="Scree Plot with 1000 Simulated Runs, 95th Percentile (random.polychor.pa)", include=FALSE, eval=FALSE}
# this is a very small pkg and produces weird results.
pa_act <- as.matrix(act_compl[,1:16])
pa_act[pa_act == "never"] <- 1
pa_act[pa_act == "sometimes"] <- 2
pa_act[pa_act == "often"] <- 3
storage.mode(pa_act) <- "integer"
library(MASS)
pa1 <- random.polychor.pa::random.polychor.pa(data.matrix = pa_act,
                                                  nrep = runs,
                                                  q.eigen = .95,
                                                  comparison = "random",
                                                  fit.pa = FALSE,
...
The `random.polychor.pa` suggests that, using polychoric correlations, between `2` and `3` factors pass a Velicer MAP criterium.

```{r paran2, fig.cap = "Scree Plot with 1000 Simulated Runs, 50th, 95th and 99th Percentile (pcaPA)", autodep = FALSE, include = FALSE}
pa2 <- pcaPA::PA(dataMatrix = as.data.frame(act_compl[, 1:16]),
                 percentiles = c(.5, .95, .99),
                 nReplicates = runs,
                 type = "ordered",
                 use = "pairwise.complete.obs",
...
r2 <- sum(pa2$observed$eigenValues[1:2]/16)
```

Also using polychoric correlations, a (more mainstream) parallel analysis (using `pcaPA::PA()`) suggests that only `2` factors can be retained (no matter the percentile).
All later principal components have Eigenvalues that are insignificantly different from those that could be expected from random, similarly parametrized data, plotted in figure \@ref(fig:paran2).
When debiased (by the random Eigenvalues), they no longer pass the Kaiser-Guttman rule of being greater than 1.
Parallel Analysis is quite sensitive to large sample sizes, tending --- if anything --- towards *over-retention* in datasets as long as BIBB/BAuA.
Therefore, the PA criterion should be taken as a strong indication to retain only two factors.
This relatively small retention may be a result of the underlying low information-theoretic value of the three-level ordinal activity ratings.

Such a model explains **`r r2*100`** percent of the total variance, a comparatively low to medium value.


### PCA results

```{r pcares, fig.cap="Loadings of Two Varimax Rotated Components"}
pcarot <- psych::principal(r = polies$correlations,
                           nfactors = 2,
                           residuals = FALSE,
                           rotate = "varimax")
loas <- as.data.frame(unclass(pcarot$loadings))
loas$var <- rownames(loas)

g <- ggplot(data = loas, mapping = aes(x = RC1, y = RC2, color = var, label = var))
g <- g + geom_point(size = 3)
g <- g + xlab(label = "RC1 (Service and Knowledge Sector) ") + xlim(-1, 1)
g <- g + ylab(label = "RC2 (Industry and Manufacturing)") + ylim(-1,1)
...
Figure \@ref(fig:pcares) gives the final result of the analysis.
It confirms the pattern initially visible from the correlation heatmap: some activities covarying around manufacturing and others loading highly on what might be called the service and knowledge economy.

Because the analysis is based on polychoric correlations, short of an IRT-approach, scores cannot be calculated.


### CFA results

(skipped for now, because results will likely be *very* similar to PCA results.)

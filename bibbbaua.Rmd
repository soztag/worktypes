...
To center an R-mode observation-column of all respondents over all their respective activities, would effectively treat the *normative* activity data as *ipsative* diary entries.
After centering, *all* respondents would have the same total sum of hours worked, distributed proportionally over their `sometimes` and `often` activities. [^zero-hours]
...
By contrast, a participant who reported `often` for eight categories, and `none` for the remaining 8, would receive `.5` for often-rated activies, and `-.5` for none. [^weight-by-hours]

[^zero-hours]: The actual mean per column would, by definition, be zero, with some entries in the negative, which seems counterintuitive for "hours worked".
    However, this is merely a linear shift, and would not cause any problems.
...
Such differences in means are not a theoretical concern.




### Ipsative vs. Normative Measurement (Input Data)
